...
add_action('wp_ajax_feasy_save_form', function () {
    ...
    function feasy_array_to_php($data, $indent = 0) {
        ...
        if (is_array($data)) {
            ...
            if (!$isAssoc && count($data) === 1 && !is_array(reset($data))) {
                return '[' . feasy_array_to_php(reset($data), 0) . ']';
            }
            ...
            foreach ($data as $key => $value) {
                $keyPart = '';
                if ($isAssoc) {
                    $pad = str_repeat(' ', $maxKeyLen - strlen($key) + 1);
                    $keyPart = "'" . addslashes($key) . "'" . $pad . "=> ";
                }
...
add_action('wp_ajax_feasy_load_logic', function () {
    ...
    $logic = include $path;
    // Normalizar en caso de que el archivo tenga la forma ['data' => [...]]
    if (is_array($logic) && array_key_exists('data', $logic) && count($logic) === 1) {
        $logic = $logic['data'];
    }

    wp_send_json_success($logic);
});
...
add_action('wp_ajax_feasy_save_logic', function () {
    ...
    // Si el JSON viene con clave "data", extraerla para mantener formato plano
