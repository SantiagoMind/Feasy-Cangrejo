...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSaveToast(message) {
        if (!saveToast) {
            ...
            saveToast.className = 'feasy-toast';
            const msg = document.createElement('span');
            msg.className = 'feasy-toast-msg';
            const close = document.createElement('button');
            close.className = 'feasy-toast-close';
            close.textContent = 'x';
            close.addEventListener('click', () => {
                saveToast.style.display = 'none';
            });
            saveToast.appendChild(msg);
            saveToast.appendChild(close);
            document.body.appendChild(saveToast);
            saveToast.msgEl = msg;
        }
        saveToast.msgEl.textContent = message;
    ...
    function markUnsaved() {
        hasUnsaved = true;
        if (saveBtn) saveBtn.classList.add("feasy-visible");
    }

    function markSaved() {
        hasUnsaved = false;
        if (saveBtn) saveBtn.classList.remove("feasy-visible");
    }
    ...
    function autosave(priority = 'medium') {
        ...
        fetch(`${feasy_globals.ajaxurl}?action=feasy_load_form&file=${encodeURIComponent(window.currentFile)}`)
            ...
            .then(data => {
                ...
                const message = data.success ? 'Last saved: ' + new Date().toLocaleTimeString() : 'Save failed';
                showSaveToast(message);
                if (data.success) markSaved();
                if (saveIndicator) {
                    saveStatus.textContent = message;
                    saveDot.textContent = data.success ? 'OK' : 'ERR';
                }
            })
            .then(() => saveLogic())
            .catch(err => {
    ...
    function renderFields() {
        ...
        window.currentFields.forEach((field, index) => {
            ...
            wrapper.querySelector('.field-name')?.addEventListener('input', () => { markUnsaved(); debouncedAutosave(); });
            wrapper.querySelector('.field-label')?.addEventListener('input', () => { markUnsaved(); debouncedAutosave(); });
            wrapper.querySelector('.field-type')?.addEventListener('change', () => { markUnsaved(); autosave('medium'); });
            wrapper.querySelector('.field-options')?.addEventListener('input', () => { markUnsaved(); debouncedAutosave(); });
            wrapper.querySelector('.field-conditional-field')?.addEventListener('input', () => { markUnsaved(); debouncedAutosave(); updateSummary(wrapper); });
            wrapper.querySelector('.field-conditional-value')?.addEventListener('input', () => { markUnsaved(); debouncedAutosave(); updateSummary(wrapper); });
            wrapper.querySelector('.field-conditional-type')?.addEventListener('change', () => { markUnsaved(); autosave('medium'); updateSummary(wrapper); });
        });
    ...
    saveBtn.addEventListener('click', () => {
        ...
        fetch(`${feasy_globals.ajaxurl}?action=feasy_load_form&file=${encodeURIComponent(window.currentFile)}`)
            ...
            .then(data => {
                ...
                showSaveToast(data.success ? 'Saved successfully' : 'Save failed');
                if (data.success) markSaved();
            })
            .catch(() => {
                alert('?? Save error');
